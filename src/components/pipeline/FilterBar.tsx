    ...
    <div className="rounded-xl border border-slate-200 bg-white p-4 dark:border-white/10 dark:bg-[#15151f]">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-3 text-xs">
        ...
        <details className="cursor-pointer">
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2 rounded-lg border border-slate-100 bg-slate-50 p-2 dark:border-white/5 dark:bg-white/5">
            ...
            <input type="number" value={filters.minPriority} onChange={(e) => set("minPriority", e.target.value)} placeholder="Min"
              className="w-16 rounded border border-slate-200 bg-white px-1.5 py-0.5 text-[11px] dark:border-slate-700 dark:bg-slate-800 dark:text-white" />
            <span className="text-slate-400">→</span>
            <input type="number" value={filters.maxPriority} onChange={(e) => set("maxPriority", e.target.value)} placeholder="Max"
              className="w-16 rounded border border-slate-200 bg-white px-1.5 py-0.5 text-[11px] dark:border-slate-700 dark:bg-slate-800 dark:text-white" />
            <span className="ml-2 text-[10px] text-slate-500">Advisory:</span>
            <input type="number" value={filters.minAdvisory} onChange={(e) => set("minAdvisory", e.target.value)} placeholder="Min"
              className="w-16 rounded border border-slate-200 bg-white px-1.5 py-0.5 text-[11px] dark:border-slate-700 dark:bg-slate-800 dark:text-white" />
            <span className="text-slate-400">→</span>
            <input type="number" value={filters.maxAdvisory} onChange={(e) => set("maxAdvisory", e.target.value)} placeholder="Max"
              className="w-16 rounded border border-slate-200 bg-white px-1.5 py-0.5 text-[11px] dark:border-slate-700 dark:bg-slate-800 dark:text-white" />
            <span className="ml-2 text-[10px] text-slate-500">Risk:</span>
            <input type="number" value={filters.minRisk} onChange={(e) => set("minRisk", e.target.value)} placeholder="Min"
              className="w-16 rounded border border-slate-200 bg-white px-1.5 py-0.5 text-[11px] dark:border-slate-700 dark:bg-slate-800 dark:text-white" />
            <span className="text-slate-400">→</span>
            <input type="number" value={filters.maxRisk} onChange={(e) => set("maxRisk", e.target.value)} placeholder="Max"
              className="w-16 rounded border border-slate-200 bg-white px-1.5 py-0.5 text-[11px] dark:border-slate-700 dark:bg-slate-800 dark:text-white" />
          </div>
        </details>
      </div>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-3 text-xs">
        ...
        <input type="date" value={filters.dateFrom} onChange={(e) => set("dateFrom", e.target.value)}
          className="rounded-lg border border-slate-200 bg-white px-2 py-1.5 outline-none focus:border-indigo-500 dark:border-slate-700 dark:bg-slate-800 dark:text-white" />
        <span className="text-slate-400">→</span>
        <input type="date" value={filters.dateTo} onChange={(e) => set("dateTo", e.target.value)}
          className="rounded-lg border border-slate-200 bg-white px-2 py-1.5 outline-none focus:border-indigo-500 dark:border-slate-700 dark:bg-slate-800 dark:text-white" />

        <span className="ml-4 text-slate-500">Value ($M):</span>
        <input type="number" value={filters.minValueM} onChange={(e) => set("minValueM", e.target.value)} placeholder="Min"
          className="w-24 rounded-lg border border-slate-200 bg-white px-2 py-1.5 outline-none focus:border-indigo-500 dark:border-slate-700 dark:bg-slate-800 dark:text-white" />
        <span className="text-slate-400">→</span>
        <input type="number" value={filters.maxValueM} onChange={(e) => set("maxValueM", e.target.value)} placeholder="Max"
          className="w-24 rounded-lg border border-slate-200 bg-white px-2 py-1.5 outline-none focus:border-indigo-500 dark:border-slate-700 dark:bg-slate-800 dark:text-white" />
      </div>
    </div>
  );
...
function Select({
  ...
  return (
    <select
      ...
      className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm outline-none focus:border-indigo-500 dark:border-slate-700 dark:bg-slate-800 dark:text-white"
    >
