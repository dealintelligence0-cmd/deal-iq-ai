...
export default function AIGenerateConfirm({
  ...
  open: boolean;
  onClose: () => void;
  onConfirm: (tier: "premium" | "economic" | "offline") => void;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="card relative max-w-lg w-full p-6">
        ...
        <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
          You&apos;re about to generate a {moduleNames[module]}. This will use tokens. Pick your tier:
        </p>
        ...
        <div className="mt-4 space-y-2">
          ...
          {premiumProvider.hasKey && premiumProvider.provider && premiumEst && (
            <button
              ...
              className="w-full rounded-lg border-2 border-purple-300 bg-gradient-to-br from-purple-50 to-indigo-50 p-3 text-left transition hover:border-purple-500 hover:shadow-sm dark:border-purple-900/50 dark:from-purple-950/30 dark:to-indigo-950/30">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 text-[11px] text-purple-800 dark:text-purple-300">
                    {PROVIDERS[premiumProvider.provider as ProviderId]?.label} · <code className="text-[10px]">{premiumProvider.model}</code>
                  </p>
                  <p className="mt-1 text-[11px] text-slate-600 dark:text-slate-400">
                    Best quality output (deeper analysis, richer tables, sector-specific insight)
                  </p>
          ...
          {economicProvider.hasKey && economicProvider.provider && economicEst && (
            <button
              onClick={() => onConfirm("economic")}
              className="w-full rounded-lg border-2 border-emerald-300 bg-emerald-50 p-3 text-left transition hover:border-emerald-500 hover:shadow-sm dark:border-emerald-900/50 dark:bg-emerald-950/20">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 text-[11px] text-emerald-800 dark:text-emerald-300">
                    {PROVIDERS[economicProvider.provider as ProviderId]?.label} · <code className="text-[10px]">{economicProvider.model}</code>
                  </p>
                  <p className="mt-1 text-[11px] text-slate-600 dark:text-slate-400">
                    Good quality at fraction of cost — Groq, Gemini Flash, DeepSeek, etc.
                  </p>
                  <p className="mt-1 text-[10px] font-mono text-slate-500">
            ...
            <button
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 text-[11px] text-slate-600 dark:text-slate-400">
                    Deterministic template — instant, free, structurally complete. No AI tokens used.
                  </p>
