...
/** Cleanse a single row. Rules applied in order. */
export function cleanseRow(row: RawDeal): CleansedDeal {
  ...
  if (!buyer) {
    ...
    exc.push({
      field: "buyer",
      severity: "info",
      message: "Buyer name normalized",
      rawValue: String(row.buyer),
      suggestedValue: buyer,
    });
  }
  cleaned.buyer = buyer;
  ...
  if (!target) {
    ...
    exc.push({
      field: "target",
      severity: "info",
      message: "Target name normalized",
      rawValue: String(row.target),
  ...
  if (buyer && target && companyKey(buyer) === companyKey(target)) {
    exc.push({
      field: "target",
      severity: "warn",
      message: "Buyer and target appear identical",
    });
  }
  ...
  if (row.sector) {
    ...
    if (sector && sector !== row.sector) {
      exc.push({
        field: "sector",
        severity: "info",
        message: "Sector standardized",
        rawValue: String(row.sector),
        suggestedValue: sector,
      });
    }
  ...
  if (row.country) {
    cleaned.country =
      String(row.country).trim().replace(/\s+/g, " ") || null;
  }
  ...
  if (row.value_raw) {
    ...
    if (v.normalizedUsd === null) {
      exc.push({
        field: "value_raw",
        severity: "warn",
        message: "Could not parse deal value",
        rawValue: String(row.value_raw),
      });
    } else if (v.normalizedUsd > 5e11) {
      exc.push({
        field: "value_raw",
        severity: "warn",
        message: "Suspiciously large value (>$500B)",
        rawValue: String(row.value_raw),
      });
      ...
      exc.push({
        field: "value_raw",
        severity: "warn",
        message: "Suspiciously small value (<$10K)",
        rawValue: String(row.value_raw),
      });
    }
  }
  ...
  if (row.stake_percent !== null && row.stake_percent !== undefined) {
    ...
    if (stake > 0 && stake <= 1) stake = stake * 100; // 0.49 → 49
    if (stake < 0 || stake > 100) {
      exc.push({
        field: "stake_percent",
        severity: "warn",
        message: "Stake outside 0–100",
        rawValue: String(row.stake_percent),
      });
  ...
  const dq =
    (requiredFilled / 3) * 0.6 +
    (optionalFilled / 6) * 0.4 -
    errorCount * 0.1;
...
/** Cleanse many rows + remove exact duplicates + flag blanks. */
